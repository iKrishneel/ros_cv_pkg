...
def non_max_suppression_equator(rect, probabilities, threshold):
        _box = []
        for box in rect:
                x1 = box.x 
                x2 = x1 + box.width
        ...
        return non_max_suppression_fast(bbox, probs, threshold);

def non_max_suppression_handler(req):
        pick = non_max_suppression_equator(
                req.rect, req.probabilities, req.threshold)
        bbox = []
        i = 0
        while i < len(pick):
                bx = pick[i][0]
