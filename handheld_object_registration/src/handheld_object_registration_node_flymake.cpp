...
void HandheldObjectRegistration::cloudCB(
    ...
    pcl::PointCloud<PointNormalT>::Ptr candidate_obj_points(
       new pcl::PointCloud<PointNormalT>);
 ...
 * NOT USED FOR NOW
 */
bool HandheldObjectRegistration::estimateObjectRegion(
    pcl::PointCloud<PointNormalT>::Ptr region_cloud,
    pcl::PointCloud<PointNormalT>::Ptr candidate_points,
    const float search_radius) {
    if (region_cloud->empty() || candidate_points->empty()) {
       ROS_ERROR("[::estimateObjectRegion]: EMPTY INPUTS");
       return false;
    }
    pcl::KdTreeFLANN<PointNormalT>::Ptr kdtree(
       new pcl::KdTreeFLANN<PointNormalT>);
    kdtree->setInputCloud(region_cloud);
    ...
    for (int i = 0; i < candidate_points->size(); i++) {
       ...
       PointNormalT pt = candidate_points->points[i];
       int search_out = kdtree->radiusSearch(
          pt, search_radius, neigbor_indices, point_squared_distance);
       
       for (int j = 0; j < neigbor_indices.size(); j++) {
          int index = neigbor_indices[j];
          temp_points->push_back(region_cloud->points[index]);
       }
    }
    
    region_cloud->clear();
    *region_cloud = *temp_points;
}
