...
void IncrementalICPRegisteration::callback(
    ...
    if (!this->prev_cloud->empty()) {
       ...
       std::cout << "Point Size: " << source->size()
                 << "\t" << target->size()  << std::endl;
       
       if (this->icpAlignPointCloud(
              this->prev_cloud, cloud, output, transformation)) {
         ...
         *reg_cloud += *output;
         ...
         // pcl::transformPointCloud(*output, *reg_cloud, global_transformation);
         // this->global_transformation = global_transformation * transformation;
         
         // pcl::copyPointCloud<PointT, PointT>(*reg_cloud, *prev_cloud);
...
bool IncrementalICPRegisteration::icpAlignPointCloud(
    ...
    // pcl::PointNormal, pcl::PointNormal> reg;
    pcl::IterativeClosestPoint<PointT, PointT> reg;
    reg.setTransformationEpsilon(1e-6);
    reg.setMaxCorrespondenceDistance(0.01f);
    ...
    reg.setEuclideanFitnessEpsilon(0.01f);
    ...
    //           <<  "\t" << reg.hasConverged() << std::endl;


    pcl::PointCloud<PointT>::Ptr reg_result(new pcl::PointCloud<PointT>);
    *reg_result = *source;
    const int max_iter = 10;
    for (int i = 0; i < max_iter; i++) {
       tmp = reg_result;
       reg.setInputSource(tmp);
       ...
       if (fabs((reg.getLastIncrementalTransformation() - prev).sum()) <
         ...
         reg.getMaxCorrespondenceDistance() - 0.001f);
       }
